...
void handle_Get() {
  ...
  for (uint8_t i = 1; i < Dispensers_Amount; i++) {
    if (i != 1) Json += ",";
    Json += "[" + String(Dispensers[i].LocationX) + "," + String(Dispensers[i].LocationY) + "," + String(Dispensers[i].TimeMSML) + "," + String(Dispensers[i].DelayAir * 5) + "," + String(Dispensers[i].IngredientID) + "," + String(Dispensers[i].FluidLevel) + "]";
  }
  Json += "],\"ingredients\":[";
