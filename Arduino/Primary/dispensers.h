 ...
 Serial.println("pingAll " + String(Dispensers_Amount));
  ...
  for (uint8_t i = 1; i < Dispensers_Amount; i++) {
    ...
    if (result == PJON_ACK) {
      ...
      //it takes 6327 microseconds to recieve and process the response, we run at 1.97kB/s
    } else
  ...
  if (length == 2) {
    switch (payload[0]) {  // Check first uint8_t as a command ID
      case DISPENSERSTATUS:
        ...
        Serial.println("Dispenser " + String(packet_info.tx.id) + " has fluid " + IngredientIDtoString(payload[1]));
        break;
      case ADOPT:
