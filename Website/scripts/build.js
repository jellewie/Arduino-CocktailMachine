...
import {readerFromStreamReader, readAll} from "https://deno.land/std@0.146.0/streams/mod.ts";
...
let gzipBuffer;
{
	...
	gzipBuffer = await readAll(readerFromStreamReader(stream.getReader()));
}

let bytesStrings = [];
{
	for (let i = 0; i < gzipBuffer.byteLength; i++) {
		const byte = gzipBuffer[i];
		const str = byte.toString(16).padStart(2, "0");
		bytesStrings.push(`0x${str}`);
	}
}
...
unsigned char CLIENT_HTML_GZIPPED[${gzipBuffer.byteLength}] = { ${bytesStrings.join(", ")} };
`;
